...
// the read that follows hits the device. Linux only, same "Linux fully
// verified, other platforms best-effort" precedent as
// system::readStickHardwareInfo(); without it a repeat run measures the
// page cache and looks inconsistently, misleadingly faster.
...
void measureRandomReads(const std::vector<std::string> &files, int reads, const application::CancellationToken &cancel,
                        domain::StickPerformanceMeasurement &out)
{
    struct Candidate
    {
        std::string path;
...
void measureSmallFiles(const std::vector<std::string> &files, std::uint64_t readBytes,
                       const application::CancellationToken &cancel, domain::StickPerformanceMeasurement &out)
{
    for (const auto &path : files) {
        dropCacheFor(path);
    }
...
{
    domain::StickPerformanceMeasurement m;
    for (const auto &path : databaseFiles) {
        std::error_code ec;
        auto size = fs::file_size(path, ec);
        if (!ec) {
            m.databaseBytes += size;
        }
    }
    m.streamingBytesPerSecond = measureStreaming(audioFiles, options.streamingBytesPerFile, cancel);
    measureRandomReads(audioFiles, options.randomReads, cancel, m);
    measureSmallFiles(smallFiles, options.smallFileReadBytes, cancel, m);
    return m;
}
