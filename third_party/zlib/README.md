...
djconvert's Linux build always uses the system's own zlib via
...
call (both djconvert's own and `third_party/libdjinterop`'s own internal,
...
with djconvert's own minimal one), tests, examples, contrib/,
